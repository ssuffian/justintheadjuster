const services = [
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-8 h-8 text-[#1a1a1a]">
...
export default function Services() {
  return (
    <section id="services" className="pt-28 pb-0 px-6 bg-[#D4B870] relative">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="text-center mb-16">
          <p className="text-[#1a1a1a]/50 text-[10px] tracking-[0.45em] uppercase font-body font-bold mb-4">What We Handle</p>
          <h2 className="font-display text-4xl md:text-5xl text-[#1a1a1a] font-bold uppercase mb-5">
            Claims We <span className="text-[#1a1a1a]/70">Fight For</span>
          </h2>
...
function ServiceCard({ service }: { service: (typeof services)[0] }) {
  return (
    <div className="service-card bg-[#C9A84C] p-5 rounded-sm relative overflow-hidden group flex gap-4 items-start">
      {/* Corner accent lines */}
      <div className="absolute top-0 left-0 w-14 h-0.5 bg-[#1a1a1a]/30" />
      <div className="absolute top-0 left-0 h-14 w-0.5 bg-[#1a1a1a]/30" />
      ...
      <div>
        {/* Title */}
        <h3 className="font-display text-lg text-[#1a1a1a] font-bold uppercase tracking-wide mb-2">
          {service.title}
        </h3>

        {/* Description */}
        <p className="text-[#1a1a1a]/75 text-sm leading-relaxed font-body">
          {service.description}
        </p>
      </div>

      {/* Bottom accent on hover */}
      <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#1a1a1a]/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
    </div>
  )
}
